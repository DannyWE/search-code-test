...
class TicketParserTest extends BaseTestSuite {

  test("should parse tickets json files") {
    val ticketsJsonStr = GetResourceContent("/tickets.json").right.get
    val errorOrTickets = TicketParser(ticketsJsonStr)

    errorOrTickets.left.map(println)

    val tickets = errorOrTickets.right.get
    tickets.length should equal(200)

    val firstTicket = tickets.head
